...
from utils import open_dataset, open_dataset_by_path, convert_dataarray_to_list
import numpy as np
import xarray as xr
...
def generate_charts(var, lat, lon, month='ann'):
    ...
    chart_series = {'observations': convert_dataarray_to_list(anusplin_location_slice)}
    ...
    chart_series['modeled_historical_median'] = convert_dataarray_to_list(
        bccaq_location_slice_historical['rcp26_{}_p50'.format(var)])
    chart_series['modeled_historical_range'] = convert_dataarray_to_list(
    ...
    for model in app.config['MODELS']:
        chart_series[model + '_median'] = convert_dataarray_to_list(bccaq_location_slice['{}_{}_p50'.format(model, var)])
        chart_series[model + '_range'] = convert_dataarray_to_list(
            xr.merge([bccaq_location_slice['{}_{}_p10'.format(model, var)],
...
def generate_spei_charts(var, lati, loni, month):
    ...
    chart_series = {}
    chart_series['observations'] = convert_dataarray_to_list(observed_location_slice)
    ...
    chart_series['modeled_historical_median'] = convert_dataarray_to_list(location_slice_historical['rcp26_spei_p50'])
    chart_series['modeled_historical_range'] = convert_dataarray_to_list(
    ...
    for model in app.config['MODELS']:
        chart_series[model + '_median'] = convert_dataarray_to_list(location_slice['{}_spei_p50'.format(model)])
        chart_series[model + '_range'] = convert_dataarray_to_list(xr.merge([location_slice['{}_spei_p10'.format(model)],
                                                                             location_slice[
...
def generate_slr_charts(lati, loni):
    ...
    for model in app.config['MODELS']:
        chart_series[model + '_median'] = convert_dataarray_to_list(location_slice['{}_slr_p50'.format(model)])
        chart_series[model + '_range'] = convert_dataarray_to_list(xr.merge([location_slice['{}_slr_p05'.format(model)],
...
def generate_regional_charts(partition, index, var, month='ann'):
    ...
    for model in app.config['MODELS']:
        chart_series[model + '_median'] = convert_dataarray_to_list(
